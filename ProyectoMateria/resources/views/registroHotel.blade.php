...
<div class="container d-flex align-items-center justify-content-center" style="min-height: 100vh;">
    ...
    <div class="col-12 col-md-10">
        ...
        <div class="card-body">
            <form action="{{ route('registroHoteles.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="txtdescripcion">{{ old('txtdescripcion') }}</textarea>
                <small class="text-danger fst-italic">{{ $errors->first('txtdescripcion') }}</small>
            </div>
            <div class="form-group">
                <label for="politicas">Políticas del Hotel</label>
                <textarea class="form-control" name="txtpoliticas">{{ old('txtpoliticas') }}</textarea>
                <small class="text-danger fst-italic">{{ $errors->first('txtpoliticas') }}</small>
            </div>
            <br>

            <button type="submit" class="btn btn-success">Registrar</button>
            <a href="{{ route('rutaCRUDhoteles') }}" class="btn btn-secondary ml-2">Cancelar</a>
            </form>
        </div>
        </div>
    </div>
